...
static Handle IonCannonTimer[MAXPLAYERS] = {null, ...};
static float ChargeUpFire[MAXPLAYERS];
static int IonCannonID[MAXPLAYERS];

static const char Energy_Sound[][] = {
	"misc/halloween/spell_meteor_impact.wav",
	"misc/halloween/hwn_plumes_capture.wav",
	"npc/combine_gunship/attack_stop2.wav"
};

public void Wand_IonCannon_MapStart()
{
...
public void Wand_IonCannon_Enable(int client, int weapon)
{
	IonCannonID[client]=EntIndexToEntRef(weapon);
	SDKUnhook(client, SDKHook_PreThink, Wand_IonCannon_Tick);
	SDKHook(client, SDKHook_PreThink, Wand_IonCannon_Tick);
}

public void Wand_IonCannon_Tick(Handle timer, DataPack pack)
{
	...
	int weapon = EntRefToEntIndex(IonCannonID[client]);
