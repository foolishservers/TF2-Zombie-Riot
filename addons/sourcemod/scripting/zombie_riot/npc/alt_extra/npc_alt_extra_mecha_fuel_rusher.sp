...
methodmap AltExtra_Mecha_Fuel_Rusher < CClotBody {
	...
	public AltExtra_Mecha_Fuel_Rusher(float vecPos[3], float vecAng[3], int ally) {
		AltExtra_Mecha_Fuel_Rusher npc = view_as<AltExtra_Mecha_Fuel_Rusher>(CClotBody(vecPos, vecAng, "models/bots/scout/bot_scout.mdl", "1.0", "1250", ally));
...
{
	Mecha_Scout npc = view_as<Mecha_Scout>(iNPC);
...
{
	Mecha_Scout npc = view_as<Mecha_Scout>(victim);
...
{
	Mecha_Scout npc = view_as<Mecha_Scout>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
