...
void AltExtra_Intruso_OnMapStart() {
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Alt Extra Intruso");
...
methodmap AltExtra_Intruso < AltExtra_Base {
	...
	public AltExtra_Intruso(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		AltExtra_Intruso npc = view_as<AltExtra_Intruso>(CClotBody(vecPos, vecAng, "models/bots/spy/bot_spy.mdl", "1.0", "750", ally, false, false, true));
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop_partner/player/items/spy/dex_belitower/dex_belitower.mdl");
