...
{
	return Headcrabmilloperator(vecPos, vecAng, team);
}
...
methodmap Headcrabmilloperator < CClotBody
{
	...
	public Headcrabmilloperator(float vecPos[3], float vecAng[3], int ally)
	{
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/all_class/witchhat/witchhat_pyro.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable2, "SetModelScale");
		
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/pyro/hw2013_hardheaded_hardware/hw2013_hardheaded_hardware.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable3, "SetModelScale");
		
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/pyro/hw2013_gristly_gumbo_pot/hw2013_gristly_gumbo_pot.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable4, "SetModelScale");
		
		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/pyro/hwn2025_industrial/hwn2025_industrial.mdl");
		SetVariantString("1.0");
...
static void ClotThink(int iNPC)
{
	...
	if(fl_ruina_battery[npc.index]>fl_ruina_battery_max[npc.index])
	{
		...
		{
			fl_ruina_battery[npc.index] = 0.0;
			Headcrabmilloperator_Spawn_Self(npc);
		}
	...
	if(fl_ruina_battery_timer[npc.index]<GameTime)
	{
		if(Zombies_Currently_Still_Ongoing < NPC_HARD_LIMIT)
		{
			...
			Headcrabmilloperator_Spawn_Minnions(npc);
		}
	}
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		Headcrabmilloperator_SelfDefense(npc, GameTime, Anchor_Id);
	}
	else
	{
		npc.StopPathing();
...
static void Headcrabmilloperator_Spawn_Minnions(Headcrabmilloperator npc)
{
	...
	spawn_index = NPC_CreateByName("npc_angryheadcrab", npc.index, pos, ang, GetTeam(npc.index));
	maxhealth = RoundToNearest(maxhealth * 0.25);
...
static void Headcrabmilloperator_Spawn_Self(Headcrabmilloperator npc)
{
	...
	spawn_index = NPC_CreateByName("npc_ruina_hmo", npc.index, pos, ang, GetTeam(npc.index));
