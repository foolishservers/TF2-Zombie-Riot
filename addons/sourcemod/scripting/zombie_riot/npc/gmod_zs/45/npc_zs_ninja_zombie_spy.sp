...
methodmap NinjaSpy < CClotBody
{
	...
	public NinjaSpy(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		...
		func_NPCDeath[npc.index] = NinjaSpy_NPCDeath;
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop_partner/player/items/spy/shogun_ninjamask/shogun_ninjamask.mdl");
			...
			if(LastSpawnDiversio < GetGameTime())
			{
				...
				for(int client_check=1; client_check<=MaxClients; client_check++)
				{
					if(IsClientInGame(client_check) && !IsFakeClient(client_check))
					{
						...
						ShowGameText(client_check, "voice_player", 1, "%t", "Infected Ninja Spy Spawn");
					}
				}
			}
