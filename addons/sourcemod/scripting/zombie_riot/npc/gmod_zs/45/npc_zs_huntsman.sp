...
static const char g_MeleeAttackSounds[] = "weapons/bow_shoot.wav";

public void ZSHuntsman_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Infected Huntsman Sniper");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_zs_huntsman");
...
{
	return ZSHuntsman(vecPos, vecAng, team);
}

methodmap ZSHuntsman < CClotBody
{	
	...
	public void PlayMeleeSound() {
		EmitSoundToAll(g_MeleeAttackSounds, this.index, _, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME, 100);
	}
	public void PlayMeleeHitSound() {
		EmitSoundToAll(g_MeleeHitSounds, this.index, _, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME, 100);
	}
	...
	public ZSHuntsman(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCThink[npc.index] = ZSHuntsman_ClotThink;
...
static void ZSHuntsman_ClotThink(int iNPC)
{
	ZSHuntsman npc = view_as<ZSHuntsman>(iNPC);
...
static Action ZSHuntsman_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	if(attacker <= 0)
		return Plugin_Continue;
		
	ZSHuntsman npc = view_as<ZSHuntsman>(victim);
...
static void ZSHuntsman_NPCDeath(int entity)
{
	ZSHuntsman npc = view_as<ZSHuntsman>(entity);
	if(!npc.m_bGib)
		npc.PlayDeathSound();	
...
static void ZSHuntsman_Particle_StartTouch(int entity, int target)
{
