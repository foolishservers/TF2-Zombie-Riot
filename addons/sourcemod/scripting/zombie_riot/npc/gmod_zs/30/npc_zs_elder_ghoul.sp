...
public void ElderGhoul_OnMapStart_NPC()
{
...
{
	return ElderGhoul(vecPos, vecAng, team);
}

methodmap ElderGhoul < CClotBody
{
	...
	{
		ElderGhoul npc = view_as<ElderGhoul>(CClotBody(vecPos, vecAng, "models/zombie_riot/gmod_zs/zs_zombie_models_1_1.mdl", "1.15", "3200", ally, false));
		...
		npc.m_flSpeed = 260.0;
		func_NPCDeath[npc.index] = ElderGhoul_NPCDeath;
		func_NPCThink[npc.index] = ElderGhoul_ClotThink;
...
public void ElderGhoul_ClotThink(int iNPC)
{
	ElderGhoul npc = view_as<ElderGhoul>(iNPC);
...
public void ElderGhoul_NPCDeath(int entity)
{
	ElderGhoul npc = view_as<ElderGhoul>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
