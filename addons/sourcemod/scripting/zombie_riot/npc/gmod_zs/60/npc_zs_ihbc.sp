...
{
	return InfectedHazardous(vecPos, vecAng, team);
}
...
methodmap InfectedHazardous < CClotBody
{
	...
	public InfectedHazardous(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = InfectedHazardous_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = InfectedHazardous_OnTakeDamage;
		func_NPCThink[npc.index] = InfectedHazardous_ClotThink;		
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/medic/sum24_hazardous_vest/sum24_hazardous_vest.mdl");
		SetVariantString("1.0");
		...
		npc.m_iWearable6 = npc.EquipItem("head", "models/player/items/medic/medic_clipboard.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable5, "SetModelScale");
