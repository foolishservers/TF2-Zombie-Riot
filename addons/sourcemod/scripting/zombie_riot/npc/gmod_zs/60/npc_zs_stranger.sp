...
public void StrangPyro_OnMapStart_NPC()
{
	NPCData data;
...
static void StrangPyro_ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		StrangerPyroSelfDefense(npc,GameTime, npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
		npc.m_flGetClosestTargetTime = 0.0;
...
static float StrangPyro_Terrified(int attacker, int victim, float &damage, int weapon)
{
	ApplyStatusEffect(attacker, victim, "Terrified", 1.0);
	return 0.0;
}

static Action StrangPyro_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
...
static void StrangPyro_NPCDeath(int entity)
{
