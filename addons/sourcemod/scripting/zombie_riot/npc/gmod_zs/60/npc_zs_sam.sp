...
static const char g_MeleeAttackSounds[] = "mvm/giant_demoman/giant_demoman_grenade_shoot.wav";

public void StoneAgeMaker_OnMapStart_NPC()
{
	NPCData data;
...
methodmap StoneAgeMaker < CClotBody
{
	...
	{
		EmitSoundToAll(g_MeleeAttackSounds, this.index, SNDCHAN_AUTO, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME - 0.2);
	}
...
static void StoneAgeMaker_ClotThink(int iNPC)
{
	...
	if(npc.m_flWeaponSwitchCooldown < GameTime)
	{
		npc.m_flWeaponSwitchCooldown = GameTime + 5.0;
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		int ActionDo = StoneAgeMakerSelfDefense(npc,GameTime, flDistanceToTarget); 
		switch(ActionDo)
		{
			case 0:
			{
...
static Action StoneAgeMaker_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
...
static void StoneAgeMaker_NPCDeath(int entity)
{
...
static int StoneAgeMakerSelfDefense(StoneAgeMaker npc, float gameTime, float distance)
{
