...
static const char g_DeathSounds[] = "zombie_riot/gmod_zs/nest/nest_break.wav";
static const char g_NestSpawnSounds[] = "zombie_riot/gmod_zs/nest/nest_spawn_enemy.wav";

#define NEST_CORE_MODEL "models/zombie_riot/gmod_zs/nest/nest.mdl"
#define NEST_CORE_MODEL_SIZE "0.4"

public void Nest_OnMapStart_NPC()
{
	NPCData data;
...
static void ClotPrecache()
{
	...
	PrecacheSound(g_NestSpawnSounds);
...
methodmap Nest < CClotBody
{
	...
	public void PlayDeathSound() {
		EmitSoundToAll(g_DeathSounds, this.index, SNDCHAN_VOICE, BOSS_ZOMBIE_SOUNDLEVEL, _, BOSS_ZOMBIE_VOLUME, 100);
	}
	public void PlayNestSpawnSound() {
		EmitSoundToAll(g_NestSpawnSounds, this.index, _, BOSS_ZOMBIE_SOUNDLEVEL, _, BOSS_ZOMBIE_VOLUME, 100);
	}
	
	property int m_icurrentwave
	{
		public get()							{ return this.m_iState; }
		public set(int TempValueForProperty) 	{ this.m_iState = TempValueForProperty; }
	}
	property float m_flPlayerScalingBuilding
	{
		public get()							{ return fl_AbilityOrAttack[this.index][0]; }
		public set(float TempValueForProperty) 	{ fl_AbilityOrAttack[this.index][0] = TempValueForProperty; }
	}
	property bool m_bAllyIsBoundToVillage
	{
	...
	public Nest(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		...
		npc.m_flPlayerScalingBuilding = ZRStocks_PlayerScalingDynamic();
		if(npc.m_flPlayerScalingBuilding > 10.0)
			npc.m_flPlayerScalingBuilding = 10.0;
		npc.m_icurrentwave = RoundToNearest(npc.m_flWaveScale * 10.0);
		npc.m_bAllyIsBoundToVillage = false;
		npc.m_flSpeed = 0.0;
...
public void Nest_ClotThink(int iNPC)
{
	...
	if(i_AttacksTillMegahit[iNPC] >= 510)
	{
		...
		if(npc.m_flAttackHappens < GameTime) //spawn enemy!
		{
			...
			IncreaseSpawnRates /= (Pow(1.14, npc.m_flPlayerScalingBuilding));
			...
			if((GetTeam(iNPC) != TFTeam_Red && npc_current_count < MaxEnemiesAllowedSpawnNext(0)) || (GetTeam(iNPC) == TFTeam_Red && npc_current_count < 6))
			{
				...
				if(spawn_index > MaxClients)
				{
					...
					{
						npc.m_bAllyIsBoundToVillage = true;
					}
					if(Construct)
					{
			...
			npc.m_flAttackHappens = GameTime + IncreaseSpawnRates;
		}
	}
	else
	{
