...
{
	return ZSZombine(vecPos, vecAng, team);
}

methodmap ZSZombine < CClotBody
{
	...
	public ZSZombine(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = ZSZombine_NPCDeath;
		func_NPCThink[npc.index] = ZSZombine_ClotThink;
		func_NPCOnTakeDamage[npc.index] = ZSZombine_OnTakeDamage;
