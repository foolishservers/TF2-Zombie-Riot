...
{
	return ZSMainHeadcrab(vecPos, vecAng, team);
}

methodmap ZSMainHeadcrab < CClotBody
{
	...
	public ZSMainHeadcrab(float vecPos[3], float vecAng[3], int ally)
	{
		ZSMainHeadcrab npc = view_as<ZSMainHeadcrab>(CClotBody(vecPos, vecAng, "models/headcrabclassic.mdl", "1.15", "3000", ally, false));
...
*/
		...
		func_NPCDeath[npc.index] = ZSMainHeadcrab_NPCDeath;
		func_NPCThink[npc.index] = ZSMainHeadcrab_ClotThink;
...
public void ZSMainHeadcrab_ClotThink(int iNPC)
{
	ZSMainHeadcrab npc = view_as<ZSMainHeadcrab>(iNPC);
...
//	PrintToChatAll("%.f",GetEntPropFloat(view_as<int>(iNPC), Prop_Data, "m_speed"));
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		ZSMainHeadcrab_AnnoyingZmainwalkLogic(npc,GetGameTime(npc.index), flDistanceToTarget, IsAbuildingNearMe); 
		ZSMainHeadcrab_SelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
...
void ZSMainHeadcrab_AnnoyingZmainwalkLogic(ZSMainHeadcrab npc, float gameTime, float distance, int IsAbuildingNearMe)
{
...
void ZSMainHeadcrab_SelfDefense(ZSMainHeadcrab npc, float gameTime, int target, float distance)
{
...
public void ZSMainHeadcrab_NPCDeath(int entity)
{
	ZSMainHeadcrab npc = view_as<ZSMainHeadcrab>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
