...
methodmap CombinePretorianGuard < CClotBody {
	...
	public void DoRangeAttack(int target, bool &chase = false, bool &reload = false) {
		...
		if (!IsValidEnemy(this.index, seenTarget)) {
			...
			if (!this.IsTargetInFiringCone(seenTarget, 20.0))
			{
				chase = true;
				return;
			}
...
static int CombinePretorianGuard_Action(CombinePretorianGuard npc, int target)
{
	...
	if (pose_yaw > -1)
	{
		...
		// Yeah. this one tho.
		vecAng[1] = -vecAng[1];
		vecAng[1] = clamp(-AngleNormalizeWithMod(UTIL_AngleDiff(AngleNormalizeWithMod(vecAng[1]), AngleNormalizeWithMod(vecAng[1] + 180.0))), -60.0, 60.0);
	...
	*/
	...
	switch (npc.m_iState)
	{
		case 3:
		{
			npc.GetBaseNPC().flMaxYawRate = 1500.0;
			npc.GetLocomotion().FaceTowards(vecTarget);
			
			bool chase, reload;
			npc.DoRangeAttack(target, chase, reload);
