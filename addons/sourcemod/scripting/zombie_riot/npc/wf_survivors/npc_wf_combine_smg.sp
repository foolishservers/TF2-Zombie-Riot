...
void Whiteflower_CombineSmg_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "W.F. Suppressor");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_wf_combine_police_smg");
...
{
	return WFCombinePoliceSmg(vecPos, vecAng, team);
}
methodmap WFCombinePoliceSmg < CClotBody
{
	...
	public WFCombinePoliceSmg(float vecPos[3], float vecAng[3], int ally)
	{
		WFCombinePoliceSmg npc = view_as<WFCombinePoliceSmg>(CClotBody(vecPos, vecAng, "models/police.mdl", "1.15", "700", ally));
		...
		func_NPCDeath[npc.index] = WFCombinePoliceSmg_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = WFCombinePoliceSmg_OnTakeDamage;
		func_NPCThink[npc.index] = WFCombinePoliceSmg_ClotThink;
...
static void WFCombinePoliceSmg_ClotThink(int iNPC)
{
	WFCombinePoliceSmg npc = view_as<WFCombinePoliceSmg>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		if(npc.m_flNextRangedSpecialAttack < GetGameTime(npc.index) && flDistanceToTarget > 62500.0 && flDistanceToTarget < 122500.0 && npc.m_flReloadDelay < GetGameTime(npc.index))
		{
			float SpeedReturn[3];
			...
			float vecTarget2[3];
			WorldSpaceCenter(npc.m_iTarget, vecTarget2);
			float VecStart[3];
...
static Action WFCombinePoliceSmg_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	WFCombinePoliceSmg npc = view_as<WFCombinePoliceSmg>(victim);
...
static void WFCombinePoliceSmg_NPCDeath(int entity)
{
	WFCombinePoliceSmg npc = view_as<WFCombinePoliceSmg>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
