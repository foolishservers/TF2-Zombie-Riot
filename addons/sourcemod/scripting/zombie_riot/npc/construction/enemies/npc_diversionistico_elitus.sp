...
void Diversionistico_Elitus_OnMapStart_NPC()
{
...
stock int Diversionistico_ElitusID_Elite()
{
	return NPCId;
}
...
{
	return Diversionistico_Elitus(vecPos, vecAng, team, data);
}

methodmap Diversionistico_Elitus < CClotBody
{
	...
	public Diversionistico_Elitus(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		Diversionistico_Elitus npc = view_as<Diversionistico_Elitus>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "750", ally, false, false, true));
		...
		func_NPCOnTakeDamage[npc.index] = Diversionistico_Elitus_OnTakeDamage;
		func_NPCThink[npc.index] = Diversionistico_Elitus_ClotThink;
		...
		npc.m_iWearable6 = npc.EquipItem("head", "models/player/items/medic/coh_medichat.mdl");
		SetVariantString("1.0");
			...
			if(LastSpawnDiversio < GetGameTime())
			{
				...
				for(int client_check=1; client_check<=MaxClients; client_check++)
				{
					if(IsClientInGame(client_check) && !IsFakeClient(client_check))
					{
						...
						ShowGameText(client_check, "voice_player", 1, "%t", "Diversionistico_Elitus Spawn");
					}
				}
			}
...
public void Diversionistico_Elitus_ClotThink(int iNPC)
{
	Diversionistico_Elitus npc = view_as<Diversionistico_Elitus>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		switch(AntiCheeseReply)
		{
			case 0:
			{
				Diversionistico_ElitusSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
			}
			case 1:
			{
				npc.m_flAttackHappens = 0.0;
				Diversionistico_ElitusSelfDefenseRanged(npc,GetGameTime(npc.index), npc.m_iTarget); 
			}
		}
	}
	else
...
public Action Diversionistico_Elitus_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Diversionistico_Elitus npc = view_as<Diversionistico_Elitus>(victim);
...
public void Diversionistico_Elitus_NPCDeath(int entity)
{
	Diversionistico_Elitus npc = view_as<Diversionistico_Elitus>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void Diversionistico_ElitusSelfDefenseRanged(Diversionistico_Elitus npc, float gameTime, int target)
{
	...
	if(gameTime > npc.m_flNextRangedAttack)
	{
		...
		ApplyStatusEffect(npc.index, target, "Anti-Waves", 5.0);
		if(IsValidEntity(npc.m_iWearable5))
			RemoveEntity(npc.m_iWearable5);
...
void Diversionistico_ElitusSelfDefense(Diversionistico_Elitus npc, float gameTime, int target, float distance)
{
