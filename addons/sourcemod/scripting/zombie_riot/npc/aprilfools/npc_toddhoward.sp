...
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Todd Howard");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_toddhoward");
...
static void ClotPrecache()
{
	...
	PrecacheSoundCustom("#zombiesurvival/aprilfools/toddhoward.mp3");
...
int Toddspeedint[MAXENTITIES];
methodmap ToddHoward < CClotBody
{
	...
	public ToddHoward(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		ToddHoward npc = view_as<ToddHoward>(CClotBody(vecPos, vecAng, COMBINE_CUSTOM_MODEL, "1.25", "25000", ally, false, false, true,true)); //giant!
		
		i_NpcWeight[npc.index] = 4;
		func_NPCFuncWin[npc.index] = view_as<Function>(Raidmode_ToddHoward_Win);
		...
		for(int client_check=1; client_check<=MaxClients; client_check++)
		{
			if(IsClientInGame(client_check) && !IsFakeClient(client_check))
			{
				...
				if(StrContains(data, "seainfection") != -1)
					...
					ShowGameText(client_check, "item_armor", 1, "%s", "TIME TO BUY SKYRIM BOYS");
			}
		}
		...
		func_NPCDeath[npc.index] = ToddHoward_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = ToddHoward_OnTakeDamage;
		func_NPCThink[npc.index] = ToddHoward_ClotThink;
		
		SDKHook(npc.index, SDKHook_OnTakeDamagePost, ToddHoward_OnTakeDamagePost);

		npc.m_iWearable1 = npc.EquipItem("weapon_bone", "models/workshop/weapons/c_models/c_scout_sword/c_scout_sword.mdl");
		SetVariantString("7.5");
...
public void ToddHoward_ClotThink(int iNPC)
{
	...
	if(Toddspeedint[npc.index] == 320)
	{
		if(HasSpecificBuff(npc.index, "Godly Motivation"))
		{
			npc.m_flSpeed = 220.0;
			Toddspeedint[npc.index] = 220;
		}
	}
	else if(Toddspeedint[npc.index] == 220)
	{
		if(!HasSpecificBuff(npc.index, "Godly Motivation"))
		{
			Toddspeedint[npc.index] = 320;
	...
	if(IsEntityAlive(npc.m_iTargetWalkTo))
	{
		...
		else if(IsValidAlly(npc.index, npc.m_iTargetWalkTo) || npc.Anger)
		{
			if((npc.Anger || flDistanceToTarget < (125.0* 125.0)) && npc.m_flAlaxiosBuffEffect < GetGameTime(npc.index) && i_RaidGrantExtra[npc.index] >= 4)
			{
				...
				ActionToTake = -1;
				ToddHowardAOEBuff(npc,GetGameTime(npc.index));
			}
		}
		else
		{
	...
	if(AllowSelfDefense)
	{
		ToddHowardSelfDefense(npc, gameTime);
	}
	...
	ToddHowardHurricane(npc, gameTime);
	ToddHowardJumpSpecial(npc, gameTime);

}
	
public Action ToddHoward_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	ToddHoward npc = view_as<ToddHoward>(victim);
...
public void ToddHoward_OnTakeDamagePost(int victim, int attacker, int inflictor, float damage, int damagetype) 
{
	ToddHoward npc = view_as<ToddHoward>(victim);
	...
	if(i_RaidGrantExtra[npc.index] <= 2)
	{
		if(Ratio <= 0.85 && npc.g_TimesSummoned < 1)
		{
			...
			npc.PlaySummonSound();
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(6.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_ranged",_, RoundToCeil(7.0 * MultiGlobalEnemy));
		}
		else if(Ratio <= 0.55 && npc.g_TimesSummoned < 2)
		{
			...
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(6.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_ranged",_, RoundToCeil(5.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(4.0 * MultiGlobalEnemy));
		}
		else if(Ratio <= 0.35 && npc.g_TimesSummoned < 3)
		{
			...
			npc.m_flDoingSpecial = GetGameTime(npc.index) + 10.0;
			npc.PlaySummonSound();
			ToddHowardSpawnEnemy(npc.index,"npc_error_ranged",_, RoundToCeil(6.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(5.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(5.0 * MultiGlobalEnemy));
		}
		else if(Ratio <= 0.20 && npc.g_TimesSummoned < 4)
		{
			...
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(5.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(5.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(8.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee", RoundToCeil(10000.0 * MultiGlobalHighHealthBoss), 1, true);		
		}
	}
	else if(i_RaidGrantExtra[npc.index] == 3)
	{
		if(Ratio <= 0.85 && npc.g_TimesSummoned < 1)
		{
			...
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(6.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(4.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_ranged",_, RoundToCeil(4.0 * MultiGlobalEnemy));
		}
		else if(Ratio <= 0.55 && npc.g_TimesSummoned < 2)
		{
			npc.g_TimesSummoned = 2;
			...
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(5.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(12.0 * MultiGlobalEnemy));
		}
		else if(Ratio <= 0.35 && npc.g_TimesSummoned < 3)
		{
			npc.g_TimesSummoned = 3;
		...
		else if(Ratio <= 0.20 && npc.g_TimesSummoned < 4)
		{
			...
			npc.PlaySummonSound();
			npc.m_flDoingSpecial = GetGameTime(npc.index) + 10.0;
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(15.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_ranged",_, RoundToCeil(2.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",RoundToCeil(10000.0 * MultiGlobalHighHealthBoss), 1, true);		
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee", RoundToCeil(10000.0 * MultiGlobalHealthBoss), RoundToCeil(2.0 * MultiGlobalEnemyBoss), true);				
		}
	}
	else if(i_RaidGrantExtra[npc.index] == 4)
	{
		...
		else if(Ratio <= 0.35 && npc.g_TimesSummoned < 3)
		{
			...
			npc.m_flDoingSpecial = GetGameTime(npc.index) + 10.0;
			ToddHowardSpawnEnemy(npc.index,"npc_error_ranged",_, RoundToCeil(6.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"nnpc_error_melee",_, RoundToCeil(12.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(2.0 * MultiGlobalEnemy));
		}
		else if(Ratio <= 0.20 && npc.g_TimesSummoned < 4)
		{
			SetEntProp(npc.index, Prop_Data, "m_iHealth", ReturnEntityMaxHealth(npc.index) / 4);
			...
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(2.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",_, RoundToCeil(8.0 * MultiGlobalEnemy));
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee",RoundToCeil(2500.0 * MultiGlobalHighHealthBoss), 1, true);		
			ToddHowardSpawnEnemy(npc.index,"npc_error_melee", RoundToCeil(125000.0 * MultiGlobalHighHealthBoss), 1, true);		
		}
	}
	else
	{
...
public void ToddHoward_NPCDeath(int entity)
{
	ToddHoward npc = view_as<ToddHoward>(entity);
	if(!BlockLoseSay2)
	{
...
void ToddHowardSpawnEnemy(int alaxios, char[] plugin_name, int health = 0, int count, bool is_a_boss = false)
{
...
void ToddHowardSelfDefense(ToddHoward npc, float gameTime)
{
...
void ToddHowardJumpSpecial(ToddHoward npc, float gameTime)
{
	if(npc.m_flNextRangedSpecialAttackHappens)
	{
		...
		if(npc.m_flNextRangedSpecialAttackHappens < gameTime)
		{
			...
			EmitSoundToAll(SOUND_TITS_LIGHTNING_ABILITY_PAP_SMITE, 0, SNDCHAN_AUTO, 100, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, ThrowPos);
			EmitSoundToAll(SOUND_TITS_LIGHTNING_ABILITY_PAP_SMITE, 0, SNDCHAN_AUTO, 100, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, ThrowPos);
			int r = 200;
			int g = 200;
			int b = 255;
...
void ToddHowardHurricane(ToddHoward npc, float gameTime)
{
...
void ToddHowardAOEBuff(ToddHoward npc, float gameTime, bool mute = false)
{
	float pos1[3];
	...
	if(npc.m_flAlaxiosBuffEffect < gameTime)
	{
		...
		for(int entitycount; entitycount<MAXENTITIES; entitycount++) //Check for npcs
		{
			if(IsValidEntity(entitycount) && entitycount != npc.index && (entitycount <= MaxClients || !b_NpcHasDied[entitycount])) //Cannot buff self like this.
			{
				if(GetEntProp(entitycount, Prop_Data, "m_iTeamNum") == GetEntProp(npc.index, Prop_Data, "m_iTeamNum") && IsEntityAlive(entitycount))
				{
					...
					if(GetVectorDistance(pos1, pos2, true) < (TODDHOWARD_BUFF_MAXRANGE * TODDHOWARD_BUFF_MAXRANGE))
					{
		...
		if(buffed_anyone)
		{
			...
			spawnRing(npc.index, TODDHOWARD_BUFF_MAXRANGE * 2.0, 0.0, 0.0, 5.0, "materials/sprites/laserbeam.vmt", r, g, b, a, 1, 1.0, 6.0, 6.1, 1);
			spawnRing_Vectors(UserLoc, 0.0, 0.0, 5.0, 0.0, "materials/sprites/laserbeam.vmt", r, g, b, a, 1, 0.75, 12.0, 6.1, 1, TODDHOWARD_BUFF_MAXRANGE * 2.0);		
			npc.AddGestureViaSequence("g_wave");
			if(!mute)
			{
				spawnRing(npc.index, TODDHOWARD_BUFF_MAXRANGE * 2.0, 0.0, 0.0, 25.0, "materials/sprites/laserbeam.vmt", r, g, b, a, 1, 0.8, 6.0, 6.1, 1);
				spawnRing(npc.index, TODDHOWARD_BUFF_MAXRANGE * 2.0, 0.0, 0.0, 35.0, "materials/sprites/laserbeam.vmt", r, g, b, a, 1, 0.7, 6.0, 6.1, 1);
				npc.PlayMeleeWarCry();
			}
		}
		else
...
public void Raidmode_ToddHoward_Win(int entity)
{
	ToddHoward npc = view_as<ToddHoward>(entity);
